...
{
    public class RenderThreadAnalyzer : AbstractTextBasedFileOutputAnalyzer
    {
        ...
        {
            if (threadData.m_AllSamples == null)
            {
                return;
            }
        ...
        private void VisitChildren(ProfilerSample sample,
            Func<ProfilerSample, bool> filter)
        {
            if (sample == null)
            {
                return;
        ...
        protected override string GetResultText()
        {
            ...
            csvStringGenerator.AppendColumn("frameIdx").AppendColumn("processCommandsTime")
                .AppendColumn("waitForCommandsTime").AppendColumn("scheduleGeometryJobTime")
                .AppendColumn("scheduleGeometryJobNum").AppendColumn("presentFrameTime").AppendColumn("guiRepaint")
                .AppendColumn("cameraRenders");

            for (int i = 0; i < maxCameraNum; ++i)
            {
                csvStringGenerator.AppendColumn("Camera" + i);
                csvStringGenerator.AppendColumn("renderTime").AppendColumn("updateDepth").AppendColumn("opaque")
                    .AppendColumn("transparent").AppendColumn("imageEffect");
            }
        ...
        protected override string FooterName => "_renderthread.csv";
    }
}